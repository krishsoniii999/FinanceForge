                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-2 p-1 bg-white/[0.04] rounded-2xl">
                    <button
                      ...
                      className={`py-3 rounded-xl text-sm font-semibold transition-all ${
                        action === 'buy'
                          ? 'bg-gain text-black shadow-lg shadow-gain/25'
                  ...
                  <div className="text-center">
                    ...
                    <div className="flex items-center justify-center gap-6">
                      <motion.button
                        ...
                        className="w-12 h-12 rounded-full bg-white/[0.06] hover:bg-white/[0.1] flex items-center justify-center transition-colors"
                        disabled={shares <= 1}
                      >
                      ...
                      >
                        <input
                          ...
                          className="w-full text-center text-5xl font-bold tabular-nums bg-transparent focus:outline-none font-display"
                          min={1}
                          max={maxShares}
                        />
                      </motion.div>
                  ...
                  <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/[0.06]">
                    <div className="flex justify-between text-sm mb-2">
                      ...
                      <span className="tabular-nums">{formatCurrency(quote?.price || 0)}</span>
                    </div>
                    <div className="flex justify-between text-sm mb-3">
                      <span className="text-text-secondary">Shares</span>
                      <span className="tabular-nums">&times; {shares}</span>
                    </div>
                    <div className="h-px bg-white/[0.08] mb-3" />
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium">
                        {action === 'buy' ? 'Total Cost' : 'Total Credit'}
                      </span>
                      <motion.span
                        key={totalCost}
                  ...
                  <div className="text-center text-xs text-text-tertiary">
                    {action === 'buy' ? (
                      <>
                        Cash available: <span className="text-text-primary">{formatCurrency(cashBalance)}</span>
                        {totalCost > cashBalance && (
                          <span className="text-loss ml-2">(Insufficient funds)</span>
                        )}
                      </>
                    ) : (
                      <>
                        Shares owned: <span className="text-text-primary">{currentHolding?.shares || 0}</span>
                        {shares > (currentHolding?.shares || 0) && (
                          <span className="text-loss ml-2">(Not enough shares)</span>
                        )}
                      </>
                    )}
                  </div>
                  ...
                  <motion.button
                    ...
                    disabled={
                      shares <= 0 ||
                      !quote ||
                      (action === 'buy' && totalCost > cashBalance) ||
                      (action === 'sell' && shares > (currentHolding?.shares || 0))
                    }
                    className={`w-full py-4 rounded-2xl font-semibold text-sm flex items-center justify-center gap-2 transition-all ${
                      action === 'buy'
                        ? 'bg-gain text-black hover:bg-gain/90 disabled:bg-gain/30 disabled:text-black/50'
                        : 'bg-loss text-white hover:bg-loss/90 disabled:bg-loss/30 disabled:text-white/50'
                    }`}
                  >
                    Review Order
                ...
                >
                  <div className="text-center py-4">
                    ...
                    <p className="text-sm text-text-secondary">
                      You're about to {action} {shares} share{shares > 1 ? 's' : ''} of {symbol}
                    </p>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/[0.06] space-y-3">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className={`font-semibold ${action === 'buy' ? 'text-gain' : 'text-loss'}`}>
                        {action.toUpperCase()}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <motion.button
                      ...
                      className="py-4 rounded-2xl font-semibold text-sm bg-white/[0.06] hover:bg-white/[0.1] transition-colors"
                    >
                      Back
                    </motion.button>
                    <motion.button
              ...
              {step === 'success' && (
                ...
                >
                  <motion.div
                    ...
                    className="w-16 h-16 mx-auto mb-4 rounded-full bg-gain/20 flex items-center justify-center"
                  >
                    <CheckCircle2 size={32} className="text-gain" />
                  </motion.div>
                  <h3 className="text-xl font-semibold mb-2">Order Executed!</h3>
                  <p className="text-sm text-text-secondary mb-6">
                    You {action === 'buy' ? 'bought' : 'sold'} {shares} share{shares > 1 ? 's' : ''} of {symbol} for {formatCurrency(totalCost)}
                  </p>
                  <motion.button
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    onClick={close}
                    className="px-8 py-3 rounded-2xl font-semibold text-sm bg-accent-blue text-white hover:bg-accent-blue/90 transition-colors"
                  >
                    Done
                  </motion.button>
                </motion.div>
              ...
              {step === 'error' && (
                ...
                >
                  <motion.div
                    ...
                    animate={{ scale: 1 }}
                    transition={{ type: 'spring', stiffness: 300, damping: 20, delay: 0.1 }}
                    className="w-16 h-16 mx-auto mb-4 rounded-full bg-loss/20 flex items-center justify-center"
                  >
                  ...
                  <div className="flex gap-3 justify-center">
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      onClick={close}
                      className="px-6 py-3 rounded-2xl font-semibold text-sm bg-white/[0.06] hover:bg-white/[0.1] transition-colors"
                    >
                      Cancel
                    </motion.button>
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      onClick={() => setStep('input')}
                      className="px-6 py-3 rounded-2xl font-semibold text-sm bg-accent-blue text-white hover:bg-accent-blue/90 transition-colors"
                    >
                      Try Again
                    </motion.button>
                  </div>
